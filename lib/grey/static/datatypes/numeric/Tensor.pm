...
class Tensor {
    ...
    my sub indicies_to_flat ($indicies, $strides) {
        my $dim = 0;
        reduce { $a + $b * $strides->[ $dim++ ] } 0, @$indicies;
    }
    ...
    method reduce_data_array ($f, $initial=undef) {
        return scalar reduce { $f->($a, $b) } ($initial // ()), @$data
    }
    ...
    method index  (@indicies) {
        die "The number of indicies must match the rank got(".(scalar @indicies).") expected(".$self->rank.")"
            if $self->rank != scalar @indicies;
        return indicies_to_flat(\@indicies, \@strides)
    }

    method dim_index (@indicies) {
        die "The number of indicies must be less than the rank got(".(scalar @indicies).") expected(".$self->rank.")"
            if $self->rank < scalar @indicies;
