...
class Matrix :isa(Tensor) {
    ...
    sub concat ($class, $a, $b) {
        die "Rows must be equal to concat (".($a->rows).") != (".($a->rows).")"
    ...
    sub stack ($class, $a, $b) {
        die "Cols must be equal to stack (".($a->cols).") != (".($a->cols).")"
    ...
    method row_indices ($x) {
        die "Coord out of bounds x(${x})" if $x > $self->height;
        return $self->index( $x, 0 ) .. $self->index( $x, $self->width )
    }

    method col_indices ($y) {
        die "Coord out of bounds x(${y})" if $y > $self->width;
