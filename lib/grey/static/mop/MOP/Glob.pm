...
class MOP::Glob {
    ...
    method get_scalar_symbol { MOP::Symbol->new( glob => $self, ref => $self->get_slot_value('SCALAR') ) }
    method get_array_symbol  { MOP::Symbol->new( glob => $self, ref => $self->get_slot_value('ARRAY')  ) }
    method get_hash_symbol   { MOP::Symbol->new( glob => $self, ref => $self->get_slot_value('HASH')   ) }
    method get_code_symbol   { MOP::Symbol->new( glob => $self, ref => $self->get_slot_value('CODE')   ) }

    method get_all_symbols (@slots) {
        @slots = qw[ SCALAR ARRAY HASH CODE ] unless @slots;
        map  { MOP::Symbol->new( glob => $self, ref => $_ ) }
